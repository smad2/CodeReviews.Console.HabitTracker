...
namespace HabitTracker
{
    ...
    {
        internal static AnalyticsResult GetHabitAnalytics(
            DateTime startDate,
            DateTime endDate,
            List<Habit> habits
        )
        ...
        {
            if (!result.HabitStats.Any())
                return;

            result.MostConsistentHabit = result
                .HabitStats.OrderByDescending(h => h.ConsistencyRate)
                .First();

            result.MostActiveHabit = result
                .HabitStats.OrderByDescending(h => h.TotalQuantity)
                .First();
        }
    }
}
